...
type
  ...
  public
    ...
    procedure FromIso8601ToDateTime(const AValue: string);
  end;
  ...
  public
    ...
    procedure FromJSON(const AJSONValue: TJSONValue); overload;
...
procedure TGBClientDatetimeHelper.FromIso8601ToDateTime(const AValue: string);
begin
  Self := Iso8601ToDateTime(AValue);
end;

function TGBClientDatetimeHelper.Iso8601ToDateTime(const AValue: string): TDateTime;
var
...
procedure TGBClientDataSetHelper.FromJSON(const AJSONValue: TJSONValue);
var
...
procedure TGBClientDataSetHelper.FromJSON(const AJSONString: string);
var
  LJSON: TJSONValue;
begin
  ...
  try
    ...
    begin
      ...
      case LField.DataType of
        ftString:
          Result.AddPair(LKey, LField.AsString);

        ftAutoInc, ftInteger, ftSmallint, ftShortint, ftCurrency,
        ftSingle, ftFloat, ftLargeint, ftBCD, ftFMTBcd, ftWord,
        ftExtended, ftLongWord:
          Result.AddPair(LKey, TJSONNumber.Create(LField.AsInteger));

        ftMemo, ftWideMemo, ftWideString:
          Result.AddPair(LKey, LField.AsWideString);

        ftBoolean:
          Result.AddPair(LKey, TJSONBool.Create( LField.AsBoolean));

        ftDate, ftTime, ftTimeStamp, ftDateTime:
          Result.AddPair(LKey, LField.AsDateTime.DateTimeToIso8601);
      end;
    end;
  except
    Result.Free;
