unit GBClient.Synapse.Compress;
...
begin
  if (DetectCompressType(inStream) = ctZipFile) then
    ...
    Result := GBClient.Synapse.GZIPUtils.unzipStream(inStream, outStream)
end;
{$Else}
function DeCompress(inStream, outStream: TStream): Boolean;
var
...
begin
  {$IfNDef USE_ZLibExGZ}
   Result := GBClient.Synapse.GZIPUtils.zipStream(inStream, outStream, cldefault, zsGZip);
  {$Else}
